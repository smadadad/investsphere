...
export default function AnalyticsPanel({ provider, account }) {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      setHistory(txs.map(tx => ({
        type: tx.investmentType === 0 ? "MoneyMarket" : tx.investmentType === 1 ? "Equity" : "Bonds",
  ...
  return (
    <div className="p-10 bg-gray-100">
      ...
      {history.map((tx, i) => (
        <div key={i}>{`${tx.type}: ${tx.amount} ETH at ${new Date(tx.timestamp * 1000).toLocaleString()}`}</div>
      ))}
    </div>
  );
}